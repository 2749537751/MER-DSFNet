...
def parse_args():
    ...
    # scheduler     ReduceLROnPlateau：当验证损失停滞时降学习率      MultiStepLR：在固定轮数降学习率，如 milestones=[30,60]
    parser.add_argument('--scheduler', default='ReduceLROnPlateau',
                        choices=['CosineAnnealingLR', 'ReduceLROnPlateau', 'MultiStepLR', 'ConstantLR'])
    parser.add_argument('--min_lr', default=1e-5, type=float,
                        help='minimum learning rate')
    
    #   ReduceLROnPlateau   仅对 ReduceLROnPlateau 有效，控制降学习率的耐心和比例
    parser.add_argument('--factor', default=0.1, type=float)
    parser.add_argument('--patience', default=4, type=int)
    
    #仅对 MultiStepLR 有效，定义学习率下降的轮数和比例。
    parser.add_argument('--milestones', default='50', type=str)
    parser.add_argument('--gamma', default=2/3, type=float)
    
    #早停  （默认：-1，即关闭） 设置为正整数（如 10），当验证损失连续 10 轮不下降时停止训练。
...
def train(config, train_loader, model, criterion, optimizer):
    ...
    def moe_load_balance_loss(model_instance):
        model_ref = model_instance.module if hasattr(model_instance, 'module') else model_instance
        router_probs = getattr(model_ref, 'last_router_probs', None)
        if router_probs is None:
            return torch.tensor(0.0, device=next(model_instance.parameters()).device)

        num_experts = router_probs.size(1)
        expert_usage = router_probs.mean(dim=0)
        uniform_target = torch.full_like(expert_usage, 1.0 / num_experts)
        return F.mse_loss(expert_usage, uniform_target)
    ...
    for input, target, _ in train_loader:
        ...
        if config['deep_supervision']:
            ...
            loss /= len(outputs)
            loss = loss + 0.05 * moe_load_balance_loss(model)
...
def main():
    ...
    for epoch in range(config['epochs']):
        ...
        if (epoch + 1) % 20 == 0:
            epoch_model_path = 'models/%s/model_epoch_%d.pth' % (config['name'], epoch + 1)
            torch.save(model.state_dict(), epoch_model_path)
            print(f"=> saved epoch model (epoch {epoch + 1})")
